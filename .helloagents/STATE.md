...
## 正在做什么
Task3 结构优化、几何增强、多 seed 实验、EMA/mountain-aware 消融、0.75 激进单模型尝试、多尺度 TTA 复评、ModelScope 上传和文档更新已完成；Task1/Task3 SwanLab 云端均已清理为报告必要 run；等待用户决定是否整合 HW2 报告或继续 Task2。
...
- 单模型继续冲分消融：EMA + mountain-aware sampling / rare-class crop 原 run 已从 SwanLab 云端清理，指标保留为 `val_mIoU=0.700604`、`mountain IoU=0.309850`，整体未超过最终 seed7 checkpoint。
...
- Task3 SwanLab 项目：`https://swanlab.cn/@youngchen/cs60003-hw2-task3`；云端保留 6 个报告必要 run，链接见 `hw2/task3/SWANLAB_RUNS.md`。
