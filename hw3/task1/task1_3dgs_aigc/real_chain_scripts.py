...
PY
test -f "{config['real_chain']['tools']['object_foreground_preprocessor']}" || {{ echo "missing object foreground preprocessor" >&2; exit 1; }}
...
def _splatfacto_context(config: dict[str, Any], run_dir: Path, target: str) -> dict[str, str]:
    ...
    preamble = ""
    if target == "object_a" and process_kind == "images":
        preamble, colmap_arg = _object_a_preprocess_preamble(config, run_dir, str(source))
        process_data_cmd += colmap_arg
    ...
    return {"preamble": preamble, "process_data_cmd": process_data_cmd}
...
python "{chain['tools']['threestudio_swanlab_runner']}" \\
  ...
  name="object_c" \\
  tag="zero123" \\
  data.image_path="{chain['data']['object_c_image']}" \\
