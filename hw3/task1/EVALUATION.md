...
| 物体 C | Zero123XL | 1200 | `exports/object_c/mesh/model.obj` / `ckpts/last.ckpt` | OBJ 33.6 MB / CKPT 151.5 MB | 单图到 3D 成功；部分角度不可见 |
...
| 当前限制 | strict 统一 3D 视频中 A 作为 splat 参与渲染，但视觉质量存在低细节区域 | 语义满足“蘑菇”，颜色偏离 prompt；需在报告中说明 | 可见角度不稳定；strict 统一 3D 视频使用 OBJ 表面采样作为可视化输入 |

## 融合表达说明

题目要求说明如何统一 AIGC mesh 与 3DGS 背景。当前实现采用 **single strict 统一链路**：`render_fused_splats.py` 直接加载 A/background 的 Gaussian splat 权重，并把 B/C 的 OBJ 顶点采样为 surface splats，在同一个 `gsplat` renderer、同一相机路径下输出 `renders/fused_splats/fused_scene.mp4`。该视频用于证明最终结果不是 2D/2.5D 图像贴片。

`fused_splats/` 是本题目三类资产统一表达的 strict 证据，单通道展示路径与相机轨迹已固定，便于和 A/B/C 方案差异对齐。
